	...
	switch(buttonObj.action) {
		...
		case enumButtonActions.OPEN_EXTERNAL_LINK:
			...
			if (buttonObj.actionParams) {
				...
				if (buttonObj.actionParams && dataContent[buttonObj.actionParams]) {
					clickAction = 'window.open(\'' + dataContent[buttonObj.actionParams].path + '\', \'' + linkName + '\')';
				}
			}
		break;
